...
print ("Path is created as " + pathimage + "/gen09")
os.mkdir(pathimage+"/gen09")
...
def sample_label_id_image(n_row, batches_done,date_string):
    ...
    if opt.genidlabel < 10:
        save_image(gen_imgs.data[opt.genidlabel],  "/content/gdrive/My Drive/TFE/images/"+date_string+"/gen"+str(opt.genidlabel)+"/gen_"+str(opt.genidlabel)+"_%d.png" % batches_done, nrow=n_row, normalize=True)
    if opt.gennumber > 0:
        toto = opt.gennumber
        numbre =[]
        for a in str(toto):
            numbre.append(gen_imgs.data[int(a)])
        save_image(numbre,  "/content/gdrive/My Drive/TFE/images/"+date_string+"/gen_"+str(opt.gennumber)+"/gen"+str(opt.gennumber)+"_%d.png" % batches_done, nrow=n_row, normalize=True)
